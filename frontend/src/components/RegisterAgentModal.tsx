...
export const RegisterAgentModal = ({ onClose }: RegisterAgentModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Agent successfully registered! You can close this window.');
      setUsername('');
      setPassword('');
    } catch (err: any) {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h2 className="font-display font-bold text-xl text-slate-900">Create New Agent</h2>
        ...
        <div className="p-6">
          ...
          {success && <div className="bg-emerald-50 text-emerald-600 px-4 py-3 rounded-lg border border-emerald-100 text-sm font-medium mb-4 shadow-sm">{success}</div>}

          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-slate-700">Agent Username</label>
              <input 
                type="text" 
                required
                value={username}
            ...
            <div className="mt-2 text-center">
              <button type="submit" className="w-full bg-brand-primary text-white font-semibold py-2.5 rounded-lg hover:bg-brand-secondary transition-colors shadow-sm disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer" disabled={loading}>
